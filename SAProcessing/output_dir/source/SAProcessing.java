...
public class SAProcessing extends PApplet {

int fieldSize = 500;
int mesh = 100;
int obstacleNums = 8;
int[][] obstacles = {{0,0,fieldSize,1},
                       {0,0,1,fieldSize},
                       {fieldSize-5,0,1,fieldSize},
                       {0,fieldSize-5,fieldSize,1},
                       {100,200,50,200},
                       {300,400,200,50},
                       {200,80,80,50},
                       {150,250,100,50}};
int chargeStationNums = 1;
int[][] chargeStation = {{1,1,50,50}};
...
class Egent {
  // Perceptions
  int id,x,y,direction,egentSize,senseSize;
  float mileage = 0.0f;
  ...
  Egent (int _id, int _init_x, int _init_y, int _init_direction, int _egentSize, int _senseSize) {
    id = _id;
    x = _init_x;
    y = _init_y;
    direction = _init_direction;
    egentSize = _egentSize;
    senseSize = _senseSize;
    next_x = _init_x;
    next_y = _init_y;
    next_direction = _init_direction;
    senseMap = new int[senseSize][senseSize];
  }

  public int sensingCollision(int _x,int _y){
    int i,j;
  ...
  public int sensingMap(int _x,int _y){
    int i,j;
    int d = fieldSize/mesh;
    int l = floor(_x/d)-senseSize/2;
    int m = floor(_y/d)-senseSize/2;
    for (i = l;i<l+senseSize;i++) {
      for (j = m;j<m+senseSize;j++) {
        if (i>=0&&j>=0&&i<mesh&&j<mesh) { // avoid null pointer
          if (obMap[i][j]!=1) obMap[i][j]=3;
          senseMap[i-l][j-m]=obMap[i][j];
        }
      }
    }
    return 0;
  ...
  public void move() {
    //wander();
    sensingMap(next_x,next_y);
    //turn();
    //goForward();
    explore();
    int ret = sensingCollision(next_x,next_y);
    if (ret==1) {
...
public void setup() {
  ...
  // make egent and map
  //Egent(_id,_init_x,_init_y,_init_direction,_egentSize,_senseSize)
  egent1 = new Egent(1,50,50,120,5,15);
  egent2 = new Egent(2,100,100,120,5,15);
  makeMap();
}
...
public void draw() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "SAProcessing" };
    if (passedArgs != null) {
